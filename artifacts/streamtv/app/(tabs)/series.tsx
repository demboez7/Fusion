...
export default function SeriesScreen() {
  ...
  const handleSearch = (text: string) => {
    ...
    const t = setTimeout(() => { setSkip(0); load(text, 0); }, 400);
    setSearchTimeout(t);
  };
  ...
  const itemWidth = (344 - 16 * 2 - 12 * 2) / 3;

  if (loading) {
    return (
      <View style={[styles.screen, { backgroundColor: colors.background }]}>
        <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
          <Text style={[styles.heading, { color: colors.foreground }]}>Series</Text>
        </View>
        <SearchBar value="" onChangeText={() => {}} placeholder="Search series..." />
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <SearchBar value={search} onChangeText={handleSearch} placeholder="Search series..." />
      {series.length === 0 ? (
        <View style={styles.empty}>
          <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No series found</Text>
        </View>
      ) : (
        <FlatList
          data={series}
          ...
          scrollEnabled={series.length > 0}
          renderItem={({ item }) => <ContentCard item={item} width={itemWidth} />}
          ListFooterComponent={
...
const styles = StyleSheet.create({
  ...
  shimmerGrid: { flexDirection: "row", flexWrap: "wrap", paddingHorizontal: 16, gap: 12 },
